...
string root = "data/cost_variance/";
string output = "results/sen_cost_detail.csv";
string ave_output = "results/sen_cost_ave.csv";
string log_output = "results/sen_cost_log.txt";
...
int main(int argc, char* argv[]){
    ...
    file << "I,K, step, gamma, instance, so_assignment, so_misplace, so_obj, best_assign, worst_assign, best_misplace, worst_misplace, best, worst, time_best, time_worst, worst_lb, worst_ub, gap, time_bound, ue_assign, ue_misplace" << endl;
    file.close();
    ofstream log_file;
    ...
    ave_file << "I,K,step,gamma,ave_so,ave_best,ave_worst,ave_ue_assign,ave_ue_misplace, ave_lb, ave_ub" << endl;
    ave_file.close();
    NbHotels = 20;
    NbTypes = 4;
    for (int step = 10; step <= 50; step += 10) {
        for (int Gamma = 0; Gamma <= 50; Gamma += 5){
            double so_ave, best_ave, worst_ave, assign_ave, misplace_ave, lb_ave, ub_ave;
            for (int instance = 1; instance <= 10; instance++) {
                string file_name = root+to_string(NbHotels) + "_" + to_string(NbTypes) + "_" + to_string(step) + "_" + to_string(Gamma)+ "_" + to_string(instance) + ".csv"; 
                demand.clear();
                capacity.clear();
                cost.clear();
                Capacity_by_k.clear();
                ...
                double worst_lb = 0;
                double time_bound = 0;
                double ue_assignment = 0;
                double ue_misplace = 0;
                ...
                double gap = (worst_ub - worst_lb)/worst_lb;

                obj.setSense(IloObjective::Maximize);
                ue_model.add(obj);
                ue_cplex.solve();
                ue_assignment = ue_cplex.getValue(expr_assignment);
                ue_misplace = ue_cplex.getValue(expr_misplacement);
                env.end();

                file.open(output, ios::out|ios::app);
                file << NbHotels << "," << NbTypes << "," << step << "," << gamma << "," << instance << "," << so_assignment << "," << so_misplace << "," << so_obj << "," << best_assign << "," << worst_assign << "," << best_misplace << "," << worst_misplace << "," << best << "," << worst << "," << time_best << "," << time_worst << "," << worst_lb << "," << worst_ub << "," << gap << "," << time_bound << "," << ue_assignment << "," << ue_misplace << endl;
                file.close();

                so_ave += 1.0/instance * (so_obj - so_ave);
                best_ave += 1.0/instance * (best - best_ave);
                worst_ave += 1.0/instance * (worst - worst_ave);
                assign_ave += 1.0/instance * (ue_assignment - assign_ave);
                misplace_ave += 1.0/instance * (ue_misplace - misplace_ave);
                lb_ave += 1.0/instance * (worst_lb - lb_ave);
                ub_ave += 1.0/instance * (worst_ub - ub_ave);
            }
            ave_file.open(ave_output, ios::out|ios::app);
            ave_file << NbHotels << "," << NbTypes << "," << step << "," << gamma << "," << so_ave << "," << best_ave << "," << worst_ave << "," << assign_ave << "," << misplace_ave << "," << lb_ave << "," << ub_ave << endl;
            ave_file.close();
        }
    }
    return 0;
