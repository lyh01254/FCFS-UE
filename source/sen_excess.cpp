...
string root = "data/excess/";
string output = "results/sen_excess_detail.csv";
string ave_output = "results/sen_excess_ave.csv";
string log_output = "results/sen_excess_log.txt";
...
int main(int argc, char* argv[]){
    ...
    file << "I,K, rate, gamma, instance, so_assignment, so_misplace, so_obj, best_assign, worst_assign, best_misplace, worst_misplace, best, worst, time_best, time_worst, worst_lb, worst_ub, gap, time_bound, ue_assign, ue_misplace" << endl;
    file.close();
    ofstream log_file;
    ...
    ave_file << "I,K,rate,gamma,ave_so,ave_best,ave_worst,ave_ue_assign,ave_ue_misplace, ave_lb, ave_ub" << endl;
    ave_file.close();
    NbHotels = 20;
    NbTypes = 8;
    for (int rate = 3; rate <= 5; rate += 1){
        double so_ave, best_ave, worst_ave, assign_ave, misplace_ave, lb_ave, ub_ave;
        for (int instance = 1; instance <= 10; instance++) {
            string file_name = root+to_string(NbHotels) + "_" + to_string(NbTypes) + "_" + to_string(rate) + "_" + to_string(instance) + ".csv"; 
            demand.clear();
            capacity.clear();
            cost.clear();
            Capacity_by_k.clear();
            ...
            file << NbHotels << "," << NbTypes << "," << rate << "," << gamma << "," << instance << "," << so_assignment << "," << so_misplace << "," << so_obj << "," << best_assign << "," << worst_assign << "," << best_misplace << "," << worst_misplace << "," << best << "," << worst << "," << time_best << "," << time_worst << "," << worst_lb << "," << worst_ub << "," << gap << "," << time_bound << "," << ue_assignment << "," << ue_misplace << endl;
            file.close();
        ...
        ave_file << NbHotels << "," << NbTypes << "," << rate << "," << gamma << "," << so_ave << "," << best_ave << "," << worst_ave << "," << assign_ave << "," << misplace_ave << "," << lb_ave << "," << ub_ave << endl;
        ave_file.close();
    }
    return 0;
}
