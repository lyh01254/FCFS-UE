...
string root = "data/long_surplus/";
string output = "results/long_detail_10.csv";
string ave_output = "results/long_ave_10.csv";
string log_output = "results/long_log_10.txt";
...
int main(int argc, char* argv[]){
    ...
    log_file << "LOG" << endl;
    log_file.close();
    for (NbHotels = 60; NbHotels <= 100; NbHotels += 20){
        for (NbTypes = 16; NbTypes <= 16; NbTypes += 2){
            for (int NbSurplus = 10; NbSurplus <= 10; NbSurplus++){
                double best_min = 999999999;
                double worst_min = 9999999999999999;
                double best_max = 0;
                double worst_max = 0;
                ...
                for (int instance = 1; instance <= 5; instance++){
                    ...
                    log_file.close();
                    string file_name = root+to_string(NbHotels) + "_" + to_string(NbTypes) + "_" +to_string(NbSurplus) + "_" + to_string(instance) + ".csv"; 
                    demand.clear();
                    capacity.clear();
                    cost.clear();
                    Capacity_by_k.clear();
